...
def _no_store_headers():
    # avoid browser reusing a previous POST/HTML
  ...
  <div class="container">
    <div class="card">
      ...
      <form action="/upload-resume" method="post" enctype="multipart/form-data">
        ...
        <textarea name="job_description" placeholder="Paste the job post to tailor keyword & section advice"></textarea>
...
async def upload_resume(
    ...
    try:
        ...
        return HTMLResponse(content=f"""
<!DOCTYPE html><html><head><meta charset="utf-8"><title>Resume Analysis</title></head>
  ...
  <div style="max-width:980px;margin:40px auto">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px">
      ...
      <div class="card">
        ...
        <p><strong>Found Keywords:</strong><br>{"".join(f'<span class="pill">{k}</span>' for k in analysis["keyword_analysis"]["industry_keywords"]) or "—"}</p>
        <p><strong>Suggested Keywords:</strong><br>{"".join(f'<span class="pill">{k}</span>' for k in analysis["keyword_analysis"]["missing_keywords"]) or "—"}</p>
