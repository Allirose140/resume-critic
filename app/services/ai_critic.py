...
def detect_industry(text: str, forced: Optional[str]) -> str:
    ...
    for label, cues in rules:
        if any(cue.lower() in t for cue in cues):
            return label
    return "unknown"
...
class AICritic:
    async def analyze_resume(self, resume_text: str, job_description: Optional[str]=None, industry: Optional[str]=None) -> Dict:
        ...
        # Domain-specific recs (no generic tech bias)
        domain_recs = {
        ...
        recs = domain_recs.get(detected, ["Tailor the top third of your resume to the target role; mirror required skills."])
