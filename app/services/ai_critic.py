...
BANKS: Dict[str, KeywordBank] = {
    ...
    # Service/Retail for fast food, cashier, barista, etc.
...
class AICritic:
    async def analyze_resume(self, resume_text: str, job_description: Optional[str] = None, industry: Optional[str] = None) -> Dict:
        ...
        if job_description:
            ...
            note = "No job description provided — suggestions & coverage are shown only when a job post is supplied."
        ...
        # ---------- scoring (rebalance) ----------
        score = 50                                              # higher, fair baseline
        score += min(12, 3 * len(sections_present))            # +3 per present section, up to +12
        score += min(10, len(found_industry_terms))            # up to +10 for keywords
        score -= min(12, 3 * len(sections_missing))            # -3 per missing section, up to -12
        if job_description and suggested:
            score -= min(5, len(suggested))                    # soft -JD miss, max -5
        score += score_red_flags(resume_text)                  # 0 to -30 for red flags
        ...
        # Tech: repo/portfolio nag only if truly absent (URLs count)
        ...
        has_repo_signal = repo_url_present or bool(token_set & repo_markers)
        ...
        return {
            ...
            "keyword_analysis": {
                ...
                "keyword_coverage": keyword_coverage,   # JD-driven coverage (or '—')
                "note": note,                           # explanation if blank
            },
